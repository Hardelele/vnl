...
export interface PatternPort {
  ...
  note: string
  /**
   * Порт слоя (#583): одна строка вместо 576 портов по клетке.
   *
   * Сворачивает их сервер: подключиться к ним по одному всё равно нельзя --
   * для этого делается жгут (#582), -- а на холсте они превращались в полосу
   * точек шириной с блок. Точка порта ведёт в первую клетку слоя, и число
   * рядом говорит, сколько их всего.
   */
  layer?: { id: string; rows: number; cols: number; cells: number }
}
...
export interface SchemeNeuron {
  ...
  inhibitory: boolean
  /**
   * Слой вместо клетки (#583): столько-то клеток одной сеткой.
   *
   * Приходит с сервера вместо 576 отдельных узлов -- иначе миниатюра
   * сетчатки полоса мусора, а раскрытый блок стена кружков. Нет поля -- узел
   * обычный, и всё, что про него знали, остаётся верным.
   */
