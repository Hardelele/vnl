 ...
 *
 * Управление временем живёт в этой же шапке (#569). Прежде оно стояло в
 * верхней полосе экрана, и владелец, глядя на песочницу, сказал прямо:
 * «попытки запустить, стоп -- я бы это к таймлайну отнёс». Он прав: «Запустить»
 * и «Сброс» двигают ровно то время, которое нарисовано здесь дорожками, а
 * стрелка «+1 мс» двигает курсор этого же таймлайна. В #504 второй транспорт
 * не заводили нарочно -- два «управления временем» на одном экране были бы
 * двумя воплощениями одного понятия; здесь не второй, а переезд: наверху его
 * не остаётся.
 *
 * Именно в шапке, а не в теле: тело сворачивается в полоску, а управление
 * временем обязано оставаться на виду во время прогона (#504). Шапка видна
 * всегда, и свёрнутая панель остаётся тем, чем была, -- строкой про прогон,
 * только теперь этот прогон из неё же и пускают.
 *
 * Настройки прогона (длительность, шаг, зерно) -- в теле, над дорожками: это
 * не управление, а числа, которые задают до пуска. Прятать их вместе с
 * дорожками не жалко -- к свёрнутой панели приходят смотреть, а не настраивать.
 */
...
export interface ActivityPanelProps {
  ...
  summary: string
  /**
   * Управление временем (#569). Стоит в шапке, а не в теле: тело сворачивается,
   * а пуск и пауза обязаны оставаться на виду во время прогона (#504).
   *
   * Необязательно: панель рисуется и там, где времени не управляют.
   */
  controls?: ReactNode
  /** Числа прогона над дорожками: их задают до пуска, а не во время него. */
  settings?: ReactNode
  children: ReactNode
}
  ...
  return (
    ...
    >
      ...
      <header className="ap-head">
        ...
        {/* Управление временем сразу за сводкой: сводка говорит, что вышло, а
            кнопки рядом -- чем это двигать. Обе строки про один и тот же
            прогон, и разносить их по разным концам экрана было не за что. */}
        {controls ? <div className="ap-controls">{controls}</div> : null}
      ...
      {open ? (
        <div className="ap-body">
          {settings ? <div className="ap-run">{settings}</div> : null}
          {children}
        </div>
      ) : null}
    </section>
