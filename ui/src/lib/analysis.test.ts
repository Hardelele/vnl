...
describe('шкалы', () => {
  ...
  it('подписывает конец прогона, когда круглый шаг до него не доходит', () => {
    // 340 мс при шаге 50 кончаются на 300, а дорожки нарисованы до 340: шкала
    // говорила про прогон неправду ровно на последние 12% растра (#552).
    expect(ticks(340, 8)).toEqual([0, 50, 100, 150, 200, 250, 300])
    expect(scaleMarks(340, 8)).toEqual([0, 50, 100, 150, 200, 250, 300, 340])
  })

  it('на круглом прогоне ничего не добавляет', () => {
    expect(scaleMarks(400, 4)).toEqual(ticks(400, 4))
  })

  it('убирает тик, который встал бы вплотную к концу', () => {
    // Шаг 100, прогон 410: подписи «400» и «410» легли бы друг на друга, а
    // длину прогона называет только вторая.
    expect(ticks(410, 8)).toEqual([0, 100, 200, 300, 400])
    expect(scaleMarks(410, 8)).toEqual([0, 100, 200, 300, 410])
  })

  it('не ломается на пустом прогоне', () => {
    expect(scaleMarks(0)).toEqual([0])
  })
