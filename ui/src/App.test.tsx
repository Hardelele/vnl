...
describe('закрытое действие', () => {
  ...
  it('отказ на создании черновика тоже уводит на вход', async () => {
    serve([
      ['/api/session', NO_LOGIN],
      ['/api/catalog', CATALOG],
      ['/api/patterns', { error: 'нужен вход', login: '/auth/login' }, 401],
    ])
    ...
    expect(went).toEqual(['/auth/login'])
    // Каталог остался на экране: прежде любой 401 подменял собой всё
    // приложение -- вместе с библиотекой, которая открыта и не при чём.
    expect(host.textContent).toContain('Feed-forward inhibition')
  })
