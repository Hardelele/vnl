...
Отменённое не пропадает: снятые шаги ложатся в стопку возврата и возвращаются
`redo`, пока не сделано новое действие (#570). До этого `undo` был дорогой в
одну сторону -- одно неверное нажатие стоило работы, и бояться кнопки было за
что.
"""
...
#: Сколько шагов держат отмена и возврат -- каждая по столько.
#:
#: Пятьдесят, и число это названо здесь, а не подобрано «сколько получится»
#: (#570). Снимки полные (разбор выше), и стопка возврата удваивает память:
#: в пределе в памяти лежит сто снимков песочницы. Снимок -- это `to_plain`
#: проекта; у самой большой схемы стенда (двадцать блоков с начинкой) он
#: порядка сотни килобайт, то есть предел стоит на десятке мегабайт на
#: открытый проект. Это дорого для сервера, где открыты сотни проектов, и
#: даром для стенда, где их единицы.
#:
#: Пятьдесят, а не «сколько угодно»: глубже пятидесяти шагов человек уже не
#: отменяет -- он открывает сохранённое. И не пять: пачка правок вроде
#: «положил десять клеток и соединил» съедает два десятка шагов за минуту, и
#: предел, до которого дотягиваются за минуту работы, -- это не предел, а
#: ловушка.
HISTORY_LIMIT = 50
...
class Project:
    ...
    def __init__(self, sandbox: Sandbox, store: Store | None = None) -> None:
        ...
        #: Снятое отменой -- то, что вернёт `redo` (#570).
        #:
        #: Отдельным списком, а не отрицательным указателем в `history`: шаг
        #: истории несёт состояние *до* себя, и ходить по одному списку в обе
        #: стороны значило бы держать в нём ещё и состояние «после», то есть
        #: тот же второй снимок, только спрятанный.
        self.future: list[Step] = []
    ...
    def undo_label(self) -> str | None:
        """Что отменится следующим «назад». `None` -- отменять нечего.

        Подпись та же, что легла в историю (`_remember`), -- она написана для
        человека («вставлен паттерн «Растормаживание»»), и просится она в
        подсказку кнопки: «Отменить» без имени действия одинаково выглядит и
        перед откатом переименования, и перед откатом разобранного блока.
        """
        return self.history[-1].label if self.history else None

    @property
    def redo_label(self) -> str | None:
        return self.future[-1].label if self.future else None
    ...
    def _forget_future(self) -> None:
        """Новое действие стирает стопку возврата.

        Общее правило редакторов, и второго ждать не надо: вернуть отменённое
        после того, как поверх него уже что-то сделали, значит склеить две
        разные истории проекта. Стирается здесь, в одном месте на все
        операции, -- не в каждой из полусотни: пропуск в одной означал бы
        «вперёд», возвращающее схему, которой в этом проекте никогда не было.
        """
        self.future.clear()
    ...
    def undo(self) -> str | None:
        ...
        Снятый шаг ложится в стопку возврата вместе с тем состоянием, которое
        он у проекта отнимает: `redo` вернёт ровно его. Подпись едет с ним же
        -- это одно и то же действие, только теперь его называют «вперёд».
        """
        if not self.history:
            return None
        step = self.history.pop()
        self.future.append(Step(label=step.label, before=self.sandbox))
        del self.future[:-HISTORY_LIMIT]
        self.sandbox = step.before
        return step.label

    def redo(self) -> str | None:
        """Вернуть отменённое. Возвращает подпись возвращённого шага (#570).

        Зеркало `undo`: шаг уходит обратно в историю вместе с состоянием до
        него, и следующее «назад» снова его отменит. Ходить так можно сколько
        угодно раз -- пока не сделано новое действие, которое стопку стирает.

        Снимок не копируется: `undo` уже отдал этот объект стопке и сам держит
        другой, а `_remember` копирует песочницу перед каждой правкой -- то
        есть общего изменяемого состояния между историей и проектом не
        возникает.
        """
        if not self.future:
            return None
        step = self.future.pop()
        self.history.append(Step(label=step.label, before=self.sandbox))
        del self.history[:-HISTORY_LIMIT]
        self.sandbox = step.before
        return step.label
