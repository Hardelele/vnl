...
class Api:
    ...
    def add_cell(self, body: dict[str, Any]) -> dict[str, Any]:
        """«В каталог»: тип клетки проекта становится каталожной записью (#567).

        До этого дорога в хранилище была одна -- `vnl cell add` с файлом
        `.vnl`. Типу, попавшему в проект из разобранного паттерна, она не
        помогает: файла с ним нет, он живёт в `sandboxes/<id>.json` внутри
        песочницы, и `target` оставался заперт в одном проекте навсегда.

        Тело называет источник, а не описывает клетку: `{"sandbox": "<id>",
        "type": "target"}`. Мембрану сюда не передают, и это то же правило, по
        которому её не принимает `add_neuron`: числа лежат в проекте, сервер
        их и берёт. Разобранная альтернатива -- присылать `pointModel` целиком
        -- означала бы, что в каталог можно положить клетку, которой в проекте
        нет, то есть маршрут «положить то, что у меня стоит» стал бы маршрутом
        «завести какую угодно клетку», а форма палитры -- редактором мембраны.

        Маршрут закрыт, как и вся песочница: класть в каталог можно только из
        своего проекта, а `_project` -- единственная дверь, за которой
        проверяют владельца (#520).

        Ответ -- весь каталог, тот же самый, что отдаёт `GET /api/cells`.
        Палитра его и перечитывает: новая запись обязана встать среди
        встроенных ровно там, где её потом будут искать глазами, а собрать это
        место из одной добавленной клетки интерфейс не может -- порядок
        держит `cells.catalog`.
        """
        sandbox_id = str(body.get("sandbox") or "")
        type_id = str(body.get("type") or "")
        if not sandbox_id or not type_id:
            raise PatternError(
                "не сказано, что класть в каталог: нужны "
                '{"sandbox": "<id>", "type": "target"}'
            )
        project = self._project(sandbox_id)
        known = project.sandbox.cell_types
        if type_id not in known:
            names = ", ".join(known) or "типов нет"
            raise PatternError(f"в проекте нет типа клетки {type_id!r} ({names})")
        cell = cells.adopt(
            known[type_id],
            str(body.get("name") or ""),
            str(body.get("note") or ""),
            cells.catalog(self.store.cells()),
            replace=bool(body.get("replace")),
            # След происхождения. У клетки из файла здесь путь к файлу, и
            # смысл тот же: через месяц должно быть понятно, откуда запись
            # взялась. Имя проекта, а не его идентификатор: идентификатор в
            # каталоге ничего не опознаёт, а имя человек сам и написал.
    ...
    def ungroup(
        self, sandbox_id: str, object_id: str, body: dict[str, Any]
    ) -> dict[str, Any]:
        """Разобрать блок на клетки, связи и типы (#532).

        Выбирать нечего: блок разбирается целиком. Имена раздаёт `free_id` на
        сервере -- он один знает, что в проекте уже занято, а столкновение
        всплыло бы иначе только на запуске.

        А места приходят из тела, как и у `arrange`: клетки обязаны встать так
        же, как стояли внутри коробки, а как они там стояли, знает только
        интерфейс -- он их и нарисовал (#554). Ключи -- имена клеток внутри
        паттерна. Тело необязательно: без мест сервер расставит клетки сам,
        и старый клиент продолжает работать.
        """
        raw = body.get("places")
        places = (
            {
        ...
        project = self._project(sandbox_id)
        project.ungroup(object_id, places)
        return api.sandbox_payload(project)
...
def routes(service: Api) -> list[Route]:
    ...
    return [
        ...
        # (какой проект, какой его тип) сказан в теле, а не в пути: класть в
        # каталог -- операция над каталогом, а песочница здесь то, откуда
        # берут, и адресом каталога ей быть незачем.
        Route(
            "POST",
            re.compile(r"^/api/cells$"),
            service.add_cell,
            wants="body",
            ok=201,
        ),
