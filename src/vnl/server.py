...
def routes(service: Api) -> list[Route]:
    ...
    return [
        ...
        # Расшифровка подписей закрыта по умолчанию, как и всё, про что не
        # сказано обратное. Открывать её незачем: спрашивает её панель свойств
        # песочницы, а туда без входа и так не заходят, -- а открытый список
        # растёт только там, где за этим приходят по ссылке.
        Route("GET", re.compile(r"^/api/glossary$"), service.glossary),
        Route(
            "POST",
