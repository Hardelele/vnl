...
def glossary_payload() -> dict[str, Any]:
    ...
    return {
        ...
        # Виды точечной модели -- тем же списком и по той же причине, что роды
        # драйва (#527). Какие мембраны симулятор действительно считает, знает
        # `ir.POINT_MODELS`, и он же отвергает вид, которого нет; список в
        # браузере был бы вторым, и новый вид появлялся бы в языке, не
        # появляясь в поле выбора.
        #
        # Имя рода -- он сам (`lif`, `adex`), и это не лень: ровно этим словом
        # вид пишется в схеме, а второе, «человеческое», имя пришлось бы потом
        # искать в тексте файла. Объясняет вид `note`, а что это за поле вообще
        # -- `cell.kind` выше.
        "models": [
            {"id": name, "note": note} for name, note in ir.POINT_MODEL_NOTES.items()
        ],
        ...
        # то, что пришло.
        # Что такое драйв и что такое запись -- отдельно от того, какого они
        # бывают рода (#502). До сих пор объяснялись только роды, и слово
        # «драйв» не было раскрыто нигде: знак на холсте прочитать было не по
        # чему. Тексты, как всегда, лежат рядом со своим предметом.
        "stimulus": protocols.STIMULUS_NOTE,
        "recording": ir.RECORDING_NOTE,
...
def _sandbox_cells(sandbox: Any) -> list[dict[str, Any]]:
    """Типы клеток самого проекта (#564).

    В проекте они заводятся двумя дорогами: кладут клетку из каталога -- и её
    тип ложится копией; разбирают блок на клетки (#532) -- и типы его снимка
    переезжают в проект (`patterns._adopt_type`). Второй дорогой в проект
    попадает `target`, которого в каталоге нет вовсе, и до сих пор наружу он
    отдавался только через `pointModel` конкретной клетки: списком их никто не
    показывал, а значит третью такую клетку положить было нечем.

    Состав полей тот же, что у клеток блока (`_block_cells`), потому что
    вопрос тот же: что это за тип и кого задевает правка его порога. Своего
    вида для «типа проекта» заводить нельзя -- это тот же тип клетки, только
    словарь, в котором он лежит, другой.

    Отличие одно: тип без клеток отсюда не выбрасывается, а у блока
    выбрасывается. У блока список отвечает на «что здесь править», и тип, ни
    на одну клетку не влияющий, в нём лишний. Здесь список отвечает на «что
    можно положить на холст», и тип, у которого последнюю клетку только что
    убрали, лежит в проекте и кладётся так же, как лежал.

    Человеческого имени и объяснения здесь нет и не появится: у типа из
    паттерна есть только идентификатор. Это и есть причина, по которой
    каталог и типы проекта показываются разными списками, а не одним.
    """
    users: dict[str, list[str]] = {}
    ...
    return [
        {
            "type": type_id,
            "neurons": users.get(type_id, []),
            "inhibitory": ir.is_inhibitory_cell(cell_type),
            "transmitter": cell_type.transmitter,
            "pointModel": _point_model(cell_type.point_model),
        }
        for type_id, cell_type in sandbox.cell_types.items()
    ]
...
def sandbox_payload(project: Any) -> dict[str, Any]:
    ...
    return {
        ...
        # Типы клеток самого проекта -- рядом с клетками, а не отдельным
        # запросом: это часть того же состояния, из которого живут все
        # представления проекта. Отдельный маршрут пришлось бы перечитывать
        # после каждого разбора блока, и палитра то знала бы про `target`, то
        # нет (#564).
        "cellTypes": _sandbox_cells(sandbox),
        "links": [
            {
                "id": link.id,
        ...
        # Возврат отменённого -- такой же факт проекта, как и отмена (#570).
        # Подписи рядом: у шага истории она человеческая («вставлен паттерн
        # «Растормаживание»»), и без неё кнопка «назад» одинаково выглядит
        # перед откатом переименования и перед откатом разобранного блока.
        # Считает их `Project`, а не браузер: история живёт там, и второго
        # места, где написано, что сейчас отменится, быть не должно.
