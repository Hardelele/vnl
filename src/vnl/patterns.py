...
class Pattern:
    ...
    def from_model(
        ...
        body.stimuli = []
        body.recordings = []
        # Заявленные числа (#501) -- утверждение о демонстрации, и уезжают они
        # не в `demo`, а никуда: сверять их можно только с тем прогоном, для
        # которого они сняты, а он описан исходником. Храни их карточка, и
        # появилось бы второе место с теми же числами -- то самое, от чего
        # оператор `expect` и заводился.
        body.expectations = []
        return Pattern(
            id=id,
            name=name,
