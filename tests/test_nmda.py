...
#: деполяризацию, про которую вопрос. Веса `0.4`/`1.0` подобраны так, что
...
#:
#: Написаны вдвое большие числа, чем были (`0.2`/`0.5`), и клетка при этом
#: получает ровно столько же (#568). До правки событийный стимул доставлялся
#: дважды, и подобраны эти веса были под удвоенную доставку: с одной доставкой
#: написанные `0.2`/`0.5` дают EPSP вдвое меньше, NMDA не выходит из-под
#: магния, плато не зажигается вовсе (отношение 1.25 вместо 1.91, надпороговое
#: время 0.0 мс) -- то есть опыт перестаёт проверять то, ради чего заведён.
#: Здесь это не «подкрутить веса под тест»: абсолютный вес в этом опыте --
#: ручка калибровки, его смысл задан фразой выше («одиночный вход даёт 3.3 мВ»
#: и не проходит), и после удвоения записанного числа клетка видит прежние
#: 3.3/3.4 мВ. Менять вместо этого пороги проверок значило бы согласиться,
#: что нелинейности больше нет.
SOURCE = """
model nmda
...
ONE = SOURCE.format(wa=0.4, wn=1.0)
FOUR = SOURCE.format(wa=1.6, wn=4.0)
#: Заведомо расходящийся прогон: столько проводимости явная схема при dt = 0.1
#: мс не держит.
BOOM = SOURCE.format(wa=0.4, wn=800.0)
