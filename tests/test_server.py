...
def test_the_whole_list_of_what_answers_without_login_fits_on_one_screen(tmp_path):
    ...
    assert open_to_anyone == {
        ...
        # Показ кадра полю -- то же управление сессией, что и подача
        # величины кнопкой, и открыт он по той же причине: схема от кадра
        # не меняется, отпечаток сети остаётся прежним (#581).
        ("POST", r"^/api/sim/([^/]+)/frames$"),
        ("GET", r"^/api/sim/([^/]+)/frames$"),
        # Встроенные образцы -- содержимое пакета, а не чья-то работа:
        # список букв секретом не является, как не является им каталог.
        ("GET", r"^/api/samples$"),
        ("DELETE", r"^/api/sim/([^/]+)$"),
    }
