...
Вкладок в левой панели песочницы четыре: «Клетки», «Библиотека», «Внешнее» и
«Объекты» — из чего собирают (типы клеток, паттерны, двери наружу) и что уже
собрано. «Клетки» — каталог типов (`vnl/cells.py`, `GET /api/cells`), из
которого на холст кладут отдельную клетку. Это не раздел библиотеки, потому что клетка не разновидность паттерна:
...
Насколько мелкое изменение физики сверка обязана замечать — сказано числом, а не
словом «чувствительная». У самих паттернов чувствительность ступенчатая: драйв
бьёт в клетку-реле, а моменты разрядов квантованы шагом `dt`, поэтому удар,
ставший сильнее на процент, за реле не виден вовсе — ни в счёте спайков, ни в
моментах. Порог держит отдельный зонд (`tests/test_sensitivity.py`): там драйв
бьёт прямо в измеряемую клетку, клетка не разряжается, и меряются непрерывные
величины — пик `g_exc` и максимум `v`. Изменение силы удара на 0.1% и больше
зонд обязан ловить, на 0.01% — вправе молчать; обе границы проверены сломом
солвера в самом тесте.
...
безразмерная величина от 0 до 1, и род решает, во что она превратится. Мотор
превращает разряды клетки в число — частоту за скользящее окно. Так схема сама
рассказывает, как понимает мир, а подключающемуся не надо знать про спайки.

Родов сенсора два, и выбор между ними — не вкусовой.

`rate` льёт поток импульсов с частотой, пропорциональной величине: 1 — заданные
100 Гц, 0.5 половина, 0 тишина. Это про аксон, который уже выстрелил: снаружи
приходит очередь готовых импульсов.

`hold` держит открытой проводимость, пока держат величину: 1 — весь вес связи,
0.5 половина, 0 закрыто. Это про сам сенсорный нейрон: каналы в ресничке
открыты, пока есть стимул. Своего числа у рода нет — «сколько откроется при
полной величине» и есть вес связи.

```
sensor touch : hold
touch -> MN.soma { weight = 0.35nS }
```

Разница видна числом. У потока при 100 Гц канал открыт пятую часть времени
(импульс раз в 10 мс, спад AMPA — 2 мс), поэтому 1 нСм потоком не доводит
клетку до порога вовсе: максимум −52.45 мВ при пороге −50. Удержание держит
весь вес, и мембрана встаёт на

```
v = (v_rest + Σg·E) / (1 + Σg)
```

то есть при покое −65 мВ и реверсале AMPA 0 мВ порог −50 мВ приходится ровно на
`g = 0.3`: 0.25 нСм держат клетку под порогом сколько угодно долго (−52.0 мВ,
ни одного разряда), 0.35 разряжают её, пока держат. Втрое меньше, чем нужно
потоку, — и это не подгонка веса, а другая форма входа.

Отпустили — канал закрывается не скачком: остаток утекает постоянной спада
рецептора, как утекает всякая проводимость. Уровень едет к клетке той же
задержкой связи, что и импульс: у двери расстояние такое же, как у всякого
другого входа.
...
vnl run examples/sensor_hold.vnl --sensor "touch=1@100,0@200"  # удержанием: 5 спайков, первый на 119.4 мс
```
...
объявляет и то, на что он отвечает, и чем действует: `rate` и `hold` отвечают
на уровень («держат единицу — идёт поток», «держат — держится проводимость»), а
место для рода, отвечающего на изменение («нажали», «отпустили»), в реестре
есть — прошлая величина доходит до рода вместе с нынешней.

Чем действует род — не придирка к слову. У проводимости (`hold`) есть реверсал:
она тянет мембрану к нему, у него же останавливается и попутно шунтирует всё
остальное. Ток не знает, где клетка, и гонит её куда угодно. Рецептор в природе
— открытые каналы, то есть проводимость; током подают из пипетки.
...
### Поле сенсоров: сеть, которая смотрит

Одна дверь — одна величина, и картинку через неё не подашь. Дело не в
количестве: у пикселя есть соседи, и вся разница между «светло» и «буква T»
лежит в том, какие из них светятся вместе.

```
sensor eye : rate { grid = 24x24, channels = gray, to = 100Hz }
neuron R[24x24] : relay
eye -> R.soma { weight = 2nS }
```

Поле — та же дверь, только величин через неё входит много сразу: `24x24` в одну
яркость — это 576 величин, `channels = rgb` — 1728, по три на пиксель. **Род при
этом прежний**: 1 — заданные 100 Гц, 0.5 — половина, 0 — тишина. Сетка не меняет
ничего в том, что происходит с одной величиной; она говорит, сколько их и кто
кому сосед. Одиночная дверь — то же поле `1x1`, и писать сетку для неё не надо.

Адресуется величина так же, как пишется: `eye.r[3,7]` — красный пиксель,
`eye.r` — канал целиком, `eye` — всё поле. Тот же адрес годится и при подаче:
`--sensor eye.r[3,7]=1` зажигает один пиксель.

`neuron R[24x24]` — слой: 576 обычных нейронов, заведённых одной строкой. Они
именно обычные — `R[3,7]` ничем не отличается от нейрона, написанного отдельной
строкой, — и всё, что знает про клетки (прогон, раскладка, отчёт, экспорт),
про слои не знает вовсе. В модели остаётся только запись о том, что клетки
заведены вместе и какой у сетки размер.

Стрелка между полем и слоем одной сетки значит **каждый к своему**: пиксель
`[3,7]` идёт в клетку `R[3,7]` и никуда больше. Ради этого слой и заводят —
иначе не нужна была бы и сетка. Остальные случаи читаются так же буквально:
слой в клетку — все к одной (так собирают сумматор), клетка в слой — одна ко
всем, слои разных сеток — отказ с обоими числами. Молча растянуть 16×16 на
24×24 значило бы выдумать соседство, которого никто не писал.

Разворачивается всё это в резолвере, а не в симуляторе: после разбора в модели
стоят 576 обычных связей, каждая со своим номером величины. Поэтому прогон,
отчёт и экспорт про сетки не знают — и не узнают, когда полей станет больше
одного.

Считаются на каждом шаге не все величины поля, а живые — ненулевые и те, у
которых не доиграна фаза. Это не оптимизация про запас: в кадре буквы светится
полторы сотни пикселей из 576, и без такого списка шаг стоил бы всей сетки
разом, то есть поле 24×24 считалось бы минутами вместо секунд.

Растр в терминале сворачивает слой в одну дорожку, а под ним пишет то, о чём
про слой и спрашивают, — не «сколько разрядов», а сколько клеток ответило:
...
R[24x24]: разрядились 158 клеток из 576, 4424 спайков (по 28 на клетку)
SUM: 28 спайков (93.3 Гц)
```

### Движок источников: программа за дверью

Кто стоит снаружи — до сих пор был человек: кнопка в интерфейсе или ключ
`--sensor`. Источник — то же самое место, занятое программой:
...
`at` — кому она, `t` — момент модельного времени в миллисекундах.

Через границу идут **числа от 0 до 1** — то же, что шло всегда, только много
разом. Декодирование PNG и JPEG, уменьшение до сетки и перевод в линейный свет
живут в программе-источнике, а ядро не знает ни про форматы файлов, ни про
экран — как не знает про кнопку. Знай оно про них, первый же новый формат
правил бы симулятор.

Подпроцесс, а не плагин на Python, по той же причине: у источника своя жизнь.
Захвату экрана нужен Pillow, датчику — драйвер, чужой модели — свой питон с
несовместимыми зависимостями. Плагин внутри процесса потащил бы всё это в
окружение VNL, ради которого в `pyproject.toml` стоит пустой `dependencies`.
Труба не требует ни общего языка, ни общей установки: пиши на чём угодно,
печатай строки.

Воспроизводимость держится на том же, на чём держалась запись `--sensor`:
пришедшее ложится в поток входа со штампом модельного времени, и прогон
переигрывается из записи спайк в спайк. Кадр без момента ложится на ноль, а
второй кадр без момента — отказ: в прогоне нет часов, и раскладывать кадры по
времени нечем, кроме выдумки.

Три программы уже лежат в `tools/`: `letters.py` — буква под сетку 24×24,
`eye.py` — файл картинки, `screen.py` — снимок экрана. Растры букв лежат рядом
текстом и правятся руками: шрифт из системы дал бы разную букву на разных
машинах, а числа в примерах и тестах обязаны сходиться у всех.
...
Яркость — это частота, и видно это числом: `--level 0.5` оставляет те же 158
клеток, но по 9 спайков вместо 28, а `--level 0.2` не зажигает ни одной — 20 Гц
по 2 нСм не доводят клетку до порога, ровно тот обрыв, из-за которого завёлся
род `hold`.

Одно свойство поля стоит знать заранее: род `rate` даёт ровный поток, и у всех
величин кадра фаза одна, поэтому 158 клеток разряжаются в один и тот же миг,
слипаясь в залп. Сумматор за слоем ловит залп и говорит «свет есть», а сколько
клеток в залпе — 110 или 212 — по его растру не прочесть. Вход тут ни при чём,
он честно принёс 212 потоков; считать «сколько» умеет не всякая схема за слоем.
...
Порядок такой, и он рассказан на экране, а не только здесь: **сперва дверь,
потом кнопка, потом прогон**. Сенсор кладут из вкладки «Внешнее» слева — там же
сказано, что нажимают его кнопкой. Положенная дверь сразу выбрана, и экран
говорит строкой, что дальше нужна кнопка и где её завести; строка исчезает,
как только кнопка появилась. В схеме, где дверей ещё нет, на месте полосы
стоит одна строка — «появятся, когда в схеме будет сенсор или мотор, их кладут
слева, из вкладки „Внешнее“». Кнопка, заведённая до запуска прогона, сама
говорит «нужен прогон»: она не сломана, ей некуда подавать.

У кнопки **две привязки, и любая может быть пустой**: сенсор, которому
она отдаёт свой бит, и мотор, который её зажигает. Список дверей приходит с
сервера вместе с состоянием проекта, выдумывать его в браузере нельзя. Нажали —
в сессию ушла единица, отпустили — ноль, и пока держат, держится единица.
Ползунков нет: вход прототипа — бит.

Привязан только сенсор — кнопка палец, как и была. Только мотор — лампочка: она
не нажимается вовсе, а горит, пока мотор отдаёт ненулевое, показывая его число с
единицей от сервера. Клавиша — второй способ нажать ту же кнопку: буква или
цифра, назначенная при заведении; стрелки остаются шагом по времени, `Delete` и
`Ctrl+D` — объектом на холсте.

### Петля: сеть трогает мир и чувствует собственное действие

Привязаны обе — кнопка нажимается **сама**: мотор отдал ненулевое, сенсор
получил единицу; мотор замолчал — ноль. Ради этого сенсоры, моторы и кнопки и
делались: без петли граница односторонняя — человек жмёт, сеть отвечает, и на
этом всё. Собирается петля на схеме `сенсор → клетка → мотор`, где сенсор и
мотор привязаны к одной кнопке: одно нажатие — и дальше сеть держит себя сама.

**Петля замыкается в браузере**, тем же маршрутом, которым кнопку нажимает палец
(`POST /api/sim/<id>/sensors`), а не внутри сессии. Причины две. Первая: петля
должна идти *через кнопку* — замкни её в симуляторе, и кнопка перестанет быть
тем, через что петля проходит, и станет показом того, что и без неё происходит.
Вторая: всё пришедшее снаружи ложится в поток входа, а он часть состояния
прогона, — значит петля воспроизводится тем же механизмом, что и палец, и
перемотка переигрывает её по записи. Петля внутри симулятора потребовала бы
второго механизма воспроизведения и разошлась бы с первым.

Цена — задержка, и её стоит назвать числом. Величина мотора приходит опросом
(150 мс реального времени), а сессия идёт со скоростью 50 модельных
миллисекунд за секунду реального: шаг фонового потока — 5 модельных мс. Значит
между разрядом клетки и единицей на сенсоре проходит **порядка 3–8 модельных
миллисекунд** (замерено на схеме `sensor → pyr → motor`: первый спайк на 51.7
мс, петля подхватила на 55.0 — 3.3 мс, повторяемо). Это порядок синаптической
задержки, и для петли «сеть себя триггернула» он уместен; но задан он опросом
интерфейса, а не сетью, и при другом темпе сессии изменится пропорционально.
Петли, которым нужна задержка в доли миллисекунды, так не собираются — их место
внутри схемы, обычной связью.

**Устойчивость.** Петля без задержки была бы генератором; здесь генератора не
выходит, и причина та же, что даёт задержку: петля не может звенеть чаще, чем
идёт опрос — больше семи переключений в секунду реального времени в поток входа
не попадёт. Ломается она с другой стороны, и ломает её **короткое окно мотора**:
окно короче межспайкового интервала гасит мотор между разрядами, петля начинает
дребезжать и затухает. На той же схеме при окне 50 мс сеть держит себя до конца
прогона (35 спайков, запись из трёх событий), при окне 5 мс — рассыпается на
пятом спайке и пишет в поток входа двенадцать событий, при 1 мс не заводится
вовсе. Правило простое: окно мотора должно быть не короче межспайкового
интервала той частоты, которую петля обязана удерживать.

**Два источника одного бита разведены явно.** Бит сенсора — это «или»: он
держится, пока его держит хоть кто-то, палец или петля. Никто не побеждает —
кнопка это контакт, и два пальца на одном контакте не спорят, кто нажал.
Практическое следствие важнее правила: отпустив кнопку, которую держит петля,
человек её не гасит, потому что гасил бы не свой бит. Видно это на самой
кнопке — у кнопки в петле знак `⟳` перед именем, и пока бит держит петля, знак
горит.

Подаёт петля только **разницу** между тем, чего хочет мотор, и тем, что сессия
уже держит на сенсоре. Не «мотор горит — шлём единицу»: ответ приходит по шесть
раз в секунду, и ровное повторение своего же значения забило бы поток входа
одинаковыми событиями, каждое из которых стирает записанное после себя будущее.
Этим же петля молчит на перемотке: там величину держит запись, разницы нет — и
переигранное прошлое не перезаписывается. Проверено прямо: перемотка назад и
пересчёт дают те же 35 спайков, спайк в спайк.
...
стоит с той же стороны границы, что и палец.

Высоту полоса берёт по делу. Пустой проект её не получает вовсе: на пустом
холсте кнопок не ищут — там смотрят в палитру. Собранная схема без дверей
получает одну строку про то, где дверь взять: эти тридцать пикселей платятся
ровно там, где вопрос «а где кнопки» уже возник, и в #562 полосы не было
именно потому, что платить их было не за что.

### Привязка видна с обеих сторон

Кнопку заводят не только из полосы. В свойствах выбранного сенсора и
выбранного мотора есть раздел «Кнопки»: какие кнопки к этой двери привязаны —
и «Сделать кнопку» рядом. Там же полем выбора спрашивается встречная дверь,
и это второй способ завести петлю: выбрал мотор в свойствах сенсора — получил
кнопку, которая и подаёт, и зажигается.

Полоса кнопок при этом остаётся, и это не два места для одного: полоса про
«нажать» — она живёт под холстом вместе с растром и горит по ответу сессии;
свойства про «связать» — какие кнопки к двери привязаны и как привязать ещё.
Нажимать из свойств нельзя нарочно: нажимают там, где видно прогон. Список
кнопок один и тот же, поэтому заведённая в свойствах появляется в полосе сразу.

**Выключенная кнопка объясняет себя на себе.** Кнопка сенсора не работает,
пока не запущен прогон, — подача идёт в прогон, а не в схему. Раньше об этом
говорила строка рядом с полосой, а сама кнопка просто серела, и серое читается
как «привязка сломалась»: так её и прочли, и пошли заводить вторую. Теперь
причина написана на самой кнопке: «нужен прогон» пунктиром вместо клавиши, а
наведение добавляет «привязка цела». Кнопка, у которой убрали дверь из схемы,
говорит другое — «двери нет»: эта причина сама не пройдёт, и путать их нельзя.

### Вкладка «Внешнее»

Сенсоры и моторы живут в своей вкладке левой панели — четвёртой, рядом с
«Клетками», «Библиотекой» и «Объектами». Сверху в ней «Положить» — что можно
завести, снизу «В схеме» — что уже заведено, со строками, которые выбирают ту
же дверь, что фигура на холсте.

Отдельная вкладка, а не угол палитры клеток, потому что дверь наружу — не
клетка и не паттерн: у неё нет ни мембраны, ни портов, ни внутренностей, а есть
сторона, которой она смотрит из схемы. Раньше сенсор лежал разделом «Граница с
миром» внутри палитры клеток, а мотор заводился только из свойств выбранной
клетки — две сущности по углам двух разных мест. Теперь раздела в палитре нет:
два места для одного — ровно та болезнь, из-за которой панель свойств однажды
стала свалкой.

Называется вкладка «Внешнее», а не «Сенсоры-моторы»: это набор того, чем схема
связана с миром, и сенсором с мотором он не кончится. Вкладка, названная двумя
своими нынешними жителями, третьего не пустит, не переименовавшись, — а
переименованная вкладка это место, которое человек однажды не нашёл.

Мотор по-прежнему заводится на выбранную клетку, и это не недоделка вкладки:
мотор без клетки не существует — он и есть «смотрю на эту точку». Пока клетка
не выбрана, «+» у мотора погашена и говорит, чего не хватает; выбрать клетку
можно, не уходя из вкладки, — щелчком по холсту.

Кнопки в панели свойств клетки («Сенсор на X», «Мотор с X») остались, и это не
второе место: там цель уже названа щелчком, и сенсор приходит сразу со
стрелкой — одинокий сенсор до клетки ничего не доносит. Второй реализации нет
ни у одной из дорог: все зовут `Project`.
...
| `examples/vision.vnl` | поле 24×24 и слой за ним: буква T зажигает 158 клеток из 576 |
| `examples/library/retina24.vnl` | блок библиотеки «Сетчатка 24×24»: поле, слой и 576 выходных портов |
...
- зрение: поле сенсоров с сеткой и каналами, слой клеток одной строкой,
  попиксельная стрелка между ними и движок внешних источников — чужая
  программа печатает кадры в stdout и становится входом прогона;
- паттерны, песочницы и хранилище: библиотека с поиском и локальный сервер
  для интерфейса;
- 791 тест у ядра и 155 у интерфейса. Зависимостей у ядра нет: Node и ELK нужны
...
- **Поле сенсоров в интерфейс не приехало.** Песочница и сессия знают дверь с
  одной величиной: кнопка, лампочка, поле `sensors` в ответе сессии. Поле там
  показывать пока нечем, и кадр в живую сессию не подашь — источник работает в
  `vnl run`. Слой клеток на схеме тоже рисуется как 576 отдельных клеток.
...
pip install -e .[tools]      # Pillow для программ-источников в tools/
```

Последняя строка нужна только тем, кто подаёт в сеть картинку или экран:
`tools/eye.py` и `tools/screen.py` читают их Pillow. Само ядро остаётся без
зависимостей — через границу идут числа, и `tools/letters.py` печатает их, не
устанавливая ничего.
