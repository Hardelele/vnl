...
морфологией и точками контактов, растр спайков, трассы и отчёт о деградации.
Ни внешних библиотек, ни graphviz, ни скриптов в странице — только SVG.
На схеме сигнал течёт слева направо: дендриты веером слева, аксон справа,
кружок на ветви — место, куда сел контакт; пунктир — нейромодулятор.
...
- визуальный отчёт `vnl view` и граф в DOT;
- 28 тестов, зависимостей у ядра нет.
...
- Страница `vnl view` только показывает: редактировать схему мышью нельзя,
  раскладка простая послойная, и на плотных сетях рёбра начнут пересекаться.
